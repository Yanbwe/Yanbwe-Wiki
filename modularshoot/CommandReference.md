...
| `gun <id>` | `id` — 枪械注册 ID（如 `modularshoot:sniper_rifle`） | 给予自身一个指定枪械物品 |
| `plugin <id> [数量]` | `id` — 插件注册 ID；`数量` — 可选，给予数量（默认 1） | 给予自身一个或多个指定插件物品 |
...
| 获取一把狙击枪 | `/modularshoot gun modularshoot:sniper_rifle` |
