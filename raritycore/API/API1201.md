...
以下接口覆盖稀有度的注册、查询、颜色获取与验证，满足大部分日常调用需求。
...
RarityCoreAPI.normalizeRarity(10); // 10 (不封顶)
```

### 进阶 API

以下接口面向 Tag 规则、视觉表现读写、开关控制、集合查询与网络同步等特定场景。
...
// 写入/移除 NBT 稀有度（raritycore 复合标签）后调用，立即失效该物品的缓存
// （ID 缓存 + NBT 缓存），避免类型级 ID 缓存快速路径返回旧值导致视觉延迟
// （NBT 缓存最长约 30-60 分钟过期）
...
int r = RarityCoreAPI.validateRarity(10); // 10 (不封顶)
