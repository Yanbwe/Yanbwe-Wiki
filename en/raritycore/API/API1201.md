...
The following interfaces cover rarity registration, queries, color retrieval and validation, covering most daily calls.
...
The following interfaces target specific scenarios: Tag rules, visual style reads/writes, switches, collection queries and network sync.
...
// Call after writing/removing NBT rarity (raritycore compound tag) to immediately
// invalidate the item's cache (ID cache + NBT cache), avoiding stale visuals from
// the type-level ID cache fast path (NBT cache expires in up to ~30-60 minutes)
...
RarityCore provides the following Forge events, listenable via `MinecraftForge.EVENT_BUS`:
...
public void onRarityQuery(RarityQueryEvent event) {
    ...
    int rarity = event.getRarity();   // current result
    event.setRarity(5);               // modify result
    String source = event.getSource(); // "registry"/"nbt"/"tag" etc.
}
```
