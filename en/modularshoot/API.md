...
| Method Signature | Parameters | Returns | Description |
|-----------------|------------|---------|-------------|
| `getGunId(ItemStack gun)` | `gun` — the gun item stack | `Optional<ResourceLocation>` | Gets the gun definition ID. Returns empty for non-gun items or when no `gun_data` component exists (changed from nullable to `Optional` in 0.3.0) |
...
| `registerPlayerAttributeSourceProvider(PlayerAttributeSourceProvider provider)` | `provider` — player attribute source provider (functional interface: `(ItemStack stack, Player viewer) → Optional<PlayerAttributeValueReader>`; returning empty means "not handled") | `void` | Registers a tooltip attribute-source resolver for player-side guns. The returned `PlayerAttributeValueReader` reads final values by logical attribute id (implementations may use synced data or a player entity). Multiple providers are consulted in registration order and the first non-empty result wins. When no provider is registered, player-side gun tooltips fall back to definition base values and show a "based on holder" note |
| `registerPlugin(ResourceLocation pluginId, PluginDefinition definition)` | `pluginId` — plugin definition ID (e.g. `mypack:rapid_affix`); `definition` — the plugin definition object | `void` | **(New in 0.3.0)** Registers a plugin via the Java API. Same semantics as `registerGun`: the ID is marked as API-registered, takes priority over same-id datapack entries, and survives `/reload`. Fits programmatic plugins (random loot affixes, dynamic modifiers, etc.) |
| `registerPluginDefinitionProvider(PluginDefinitionProvider provider)` | `provider` — dynamic plugin definition provider (functional interface: `ResourceLocation → Optional<PluginDefinition>`; returning empty means "not handled") | `void` | **(New in 0.3.0)** Registers a dynamic plugin definition provider. `getPlugin` lookup order: Java API registration → providers → datapack registry. Provider-defined IDs are not listed by enumeration APIs (computed at query time). The provider runs independently on each side, so definitions must be deterministically producible on both ends |
...
The framework sync packets carry only position/direction/visual style. Mods that need to attach custom per-bullet runtime fields (spin speed, orientation angles, custom timers, etc.) can register an extension provider with `org.yanbwe.modularshoot.network.BulletSyncExtraRegistry` instead of building their own networking:

| Method Signature | Parameters | Returns | Description |
|-----------------|------------|---------|-------------|
| `BulletSyncExtraRegistry.register(BulletSyncExtraProvider provider)` | `provider` — extension-bytes provider (functional interface: `BulletRecord → byte[]`; return `null`/empty when this bullet carries no data) | `int` (provider wire index) | Registers an extension provider. Re-registering the same instance is idempotent and returns the unchanged index |
| `BulletSyncExtraRegistry.split(byte[] payload)` | `payload` — the extension payload read from a client render object | `Map<Integer, byte[]>` (provider index → bytes) | Splits the extension payload; a corrupted/truncated payload degrades to an empty map |

**Semantics and conventions**:

- When syncing each bullet, the server collects every provider's output in registration order and ships it with both full and delta entries (only 1 extra byte when nothing is contributed)
- On the client, read it via `BulletRenderManager.getRenderObject(bulletId).getExtra()`, or directly from the render object inside an `ON_VISUAL_TICK` hook; use `split` to retrieve your own bytes by index
- A throwing provider is logged and skipped; corrupted payloads degrade to an empty map and never break client rendering (consistent with the framework's degradation philosophy)
- Both sides must register providers in the same relative order (the index is the wire identity) — naturally satisfied when both sides share the mod list; the dynamic byte *content* need not match across sides (server-authoritative)

**Example (spin-speed sync)**:

```java
// Mod initialisation (both sides)
public static final int SPIN_INDEX = BulletSyncExtraRegistry.register(bullet -> {
    ByteBuffer buf = ByteBuffer.allocate(4).order(ByteOrder.LITTLE_ENDIAN);
    buf.putFloat(MyMod.getSpinSpeed(bullet)); // your server-side data source
    return buf.array();
});

// Client consumption (e.g. inside an ON_VISUAL_TICK hook)
...
| `getAttributeMount(ItemStack gun, RegistryAccess access)` | `gun` — the gun item stack; `access` — runtime registry view | `Optional<AttributeMount>` | Looks up the declared attribute mount (`ITEM` / `PLAYER`) for a gun stack. Returns `Optional.empty()` for non-guns or a missing definition; `PLAYER` means the framework does not manage that gun's item-side attribute modifier component, transferring mounting responsibility to the declaring side |
...
| Method Signature | Parameters | Returns | Description |
|-----------------|------------|---------|-------------|
| `getState(ItemStack gun, Player player)` | `gun` — gun item stack; `player` — player context (for resolving runtime registries) | `Optional<GunState>` | Gets a per-gun state read/write view for the gun instance. Returns empty for non-gun items (changed from nullable to `Optional` in 0.3.0). Typed accessors: `getInt`/`setInt`/`getLong`/`setLong`/`getDouble`/`setDouble`/`getFloat`/`setFloat`/`getBoolean`/`setBoolean`/`getString`/`setString`/`getUuid`/`setUuid`/`hasState`/`clearState` |
