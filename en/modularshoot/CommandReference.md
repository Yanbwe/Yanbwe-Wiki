...
| `gun <id>` | `id` — gun registry ID (e.g. `modularshoot:sniper_rifle`) | Gives yourself a specified gun item |
| `plugin <id> [count]` | `id` — plugin registry ID; `count` — optional, quantity to give (default 1) | Gives yourself one or more specified plugin items |
...
| Get a sniper rifle | `/modularshoot gun modularshoot:sniper_rifle` |
